...
$branch = mysqli_real_escape_string($conn,$_POST['branch']);
 ...
 $advancefunds = mysqli_real_escape_string($conn,$_POST['advancefunds']);
 $balance = $funds-$advancefunds;
 
  $q1 = $conn->query("INSERT INTO customer (sname,joindate,contact,about,emailid,branch,balance,funds) VALUES ('$sname','$joindate','$contact','$about','$emailid','$branch','$balance','$funds')") ;
  
  $sid = $conn->insert_id;
  
 $conn->query("INSERT INTO  funds_transaction (stdid,paid,submitdate,transcation_remark) VALUES ('$sid','$advancefunds','$joindate','$remark')") ;
 ...
 $id = mysqli_real_escape_string($conn,$_POST['id']);	
   $sql = $conn->query("UPDATE  customer  SET  branch  = '$branch', address  = '$address', detail  = '$detail'  WHERE  id  = '$id'");
...
{
$errormsg = "<div class='alert alert-success'> <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Success!</strong> customer Add successfully</div>";
}else if(isset($_REQUEST['act']) && @$_REQUEST['act']=="2")
{
$errormsg = "<div class='alert alert-success'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Success!</strong> customer Edit successfully</div>";
}
else if(isset($_REQUEST['act']) && @$_REQUEST['act']=="3")
{
$errormsg = "<div class='alert alert-success'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Success!</strong> customer Delete successfully</div>";
}
		...
		?>
			...
			<script type="text/javascript" src="js/validation/jquery.validate.min.js"></script>
               ...
               <div class="panel panel-primary">
						...
						<div class="form-group">
								<label class="col-sm-2 control-label" for="Old">Total funds* </label>
								<div class="col-sm-10">
									<input type="text" class="form-control" id="funds" name="funds" value="<?php echo $funds;?>" <?php echo ($action=="update")?"disabled":""; ?>  />
								</div>
						</div>
						...
						<div class="form-group">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" id="advancefunds" name="advancefunds" readonly   />
								</div>
						</div>
						<?php
						}
...
yearRange: "1970:<?php echo date('Y');?>"
		...
		if( fee!='' && !isNaN(fee))
		{
		$("#advancefunds").removeAttr("readonly");
		$("#balance").val(fee);
		$('#advancefunds').rules("add", {
            max: parseInt(fee)
        });
		
		}
		else{
		$("#advancefunds").attr("readonly","readonly");
		}
		...
		$("#advancefunds").keyup( function(){
		
		var advancefunds = parseInt($.trim($(this).val()));
		var totalfee = parseInt($("#funds").val());
		if( advancefunds!='' && !isNaN(advancefunds) && advancefunds<=totalfee)
		{
		var balance = totalfee-advancefunds;
		$("#balance").val(balance);
                            ...
                            <div class="table-sorting table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="tSortable22">
									...
									echo '<tr '.(($r['balance']>0)?'class="danger"':'').'>
											...
											<a href="customer.php?action=edit&id='.$r['id'].'" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-edit"></span></a>
