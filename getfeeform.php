...
$sid = (isset($_POST['customer']))?mysqli_real_escape_string($conn,$_POST['customer']):'';

 $sql = "select s.id,s.sname,s.balance,s.funds,s.contact,b.branch,s.joindate from customer as s,branch as b where b.id=s.branch and  s.delete_status='0' and s.id='".$sid."'";
$q = $conn->query($sql);
if($q->num_rows>0)
{

$res = $q->fetch_assoc();
echo '  <form class="form-horizontal" id ="signupForm1" action="funds.php" method="post">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" name="totalfee" id="totalfee"   value="'.$res['funds'].'" disabled />
    </div>
  </div>
...
$sid = (isset($_POST['customer']))?mysqli_real_escape_string($conn,$_POST['customer']):'';
$sql = "select paid,submitdate,transcation_remark from funds_transaction  where stdid='".$sid."'";
$fq = $conn->query($sql);
if($fq->num_rows>0)
{


 $sql = "select s.id,s.sname,s.balance,s.funds,s.contact,b.branch,s.joindate from customer as s,branch as b where b.id=s.branch  and s.id='".$sid."'";
$sq = $conn->query($sql);
$sr = $sq->fetch_assoc();
